  ...
  <div class="market-wrap">
    <b-container>
      <div>
        <b-jumbotron
          header="NFT Minter"
          lead="Mint NFT on NEAR with a click of a button"
        >
          <b-button
            v-if="!isConnect"
            @click="$bvModal.show('mint')"
            variant="primary"
            href="#"
            >Mint NFT</b-button
          >
      ...
      <div class="is-flex space-between">
        ...
        <ul class="social-list">
          <li>
            <a href="https://twitter.com/spiritbrother" target="_blank">
              <svg
                class="twitter"
                width="26"
                height="26"
                ...
                <defs>
                  <clipPath id="clip0_77:789">
                    <rect
                      ...
                      height="15.6"
                      fill="white"
                      transform="translate(5.19995 5.19995)"
                    />
                  </clipPath>
                </defs>
              </svg>
      ...
      <div class="mt-3 mb-3">
        <b-row class="is-multiline mb-3">
          <b-col class="is-flex justify-content-md-center">
            <b-card
              class="card-item mb-2 py-3 px-2 mr-2"
              align="center"
              no-body
            >
              <b-card-text class="mb-0 t-size">NAME </b-card-text>
              <b-card-text v-if="!loadingData">{{ ftName }}</b-card-text>
              <b-card-text v-else><b-skeleton></b-skeleton></b-card-text>
            </b-card>
            <b-card class="card-item mb-2 py-3 px-2" align="center" no-body>
              <b-card-text class="mb-0 t-size">SYMBOL</b-card-text>
              <b-card-text>{{ ftSymbol }}</b-card-text>
            </b-card>
          </b-col>
        </b-row>
          ...
          <b-col class="is-flex space-between">
            <div class="is-flex space-between group-filter">
              <div class="search-input">
                ...
                <b-form-input
                  placeholder="Search NFT Name or ID"
                  type="search"
                  id="search-nft"
                  v-model="search"
            ...
            <div class="icon-position">
              <div class="is-flex space-between grid-layout">
                <b-input-group-text
                  v-on:click="addClass1"
                  :class="{ active: isAddClass1 }"
                >
                  <b-icon-grid-fill></b-icon-grid-fill>
                </b-input-group-text>
                <b-input-group-text
                  v-on:click="addClass2"
                  :class="{ active: isAddClass2 }"
                >
        ...
        <b-row
          class="is-multiline filter-wrap mb-3"
          v-if="allData.length === 0 && loadingData"
        >
          <b-col class="is-flex space-between">
            <div class="is-flex space-between group-filter">
        ...
        <div
          class="mb-4 mt-4 is-flex space-between"
          v-if="!loadingData && allData.length > 0"
        >
          <div class="atribute-filter is-flex">
            <span v-for="k in Object.keys(filters)" :key="k">
              <span v-for="(v, b) in Object.keys(filters[k])" :key="b">
                <b-form-tag
                  class="mr-1 badge-custom"
                  v-if="filters[k][v]"
                  @remove="
                    filters[k][v] = false;
                    handleFilterOk();
                  "
                  >{{ `${k.toUpperCase()}:${v}` }}</b-form-tag
                >
              </span>
            </span>
            <b-form-tag
              ...
              @remove="
                ...
                handleFilterOk();
              "
              >{{ `SEARCH:${search}` }}</b-form-tag
            >
            <b-link
              type="link"
              class="filter-clear is-flex"
              v-if="totalSupply.toNumber() !== totalResults"
              @click="clearTag()"
          ...
          <span class="result" v-if="!loadingItems"
        ...
        <b-row class="is-multiline" v-bind:class="`${classActive}`">
            ...
            >
              <router-link :to="`/${it.key}`">
                <b-card-img
                  ...
                  style="background-color: #638596;height:246px;"
                  v-bind:src="
                    it.value.metadata.media.indexOf('ipfs') > -1 ? it.value.metadata.media :
                    `data:image/svg+xml;charset=UTF-8,%3Csvg width='200' height='200' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 200 200' preserveAspectRatio='none'%3E%3Cdefs%3E%3Cstyle type='text/css'%3E%23holder_17cb7089c7d text %7B fill:'%23DFE1E6';font-weight:bold;font-family:Poppins, monospace;font-size:50pt %7D %3C/style%3E%3C/defs%3E%3Cg id='holder_17cb7089c7d'%3E%3Crect width='200' height='200' fill='%23EBECF0'%3E%3C/rect%3E%3Cg%3E%3Ctext x='44.421875' y='118.5' fill='%23DFE1E6'%3E%23${it.key}%3C/text%3E%3C/g%3E%3C/g%3E%3C/svg%3E`
                  "
                  alt="image"
                ></b-card-img>
                <b-card-body class="is-flex">
                  <div class="nft-head">
                    <b-card-text class="nft-acc">{{ it.value.owner_id }}</b-card-text>
                    <b-card-title class="nft-title highlight-text-color">{{
                      `${it.value.metadata.title} #${it.key}`
                    }}</b-card-title>
                  </div>
                  <div class="nft-price" >
                    <b-card-text class="nft-price-text"
                      >{{ it.value.metadata.name}}</b-card-text
                    >
                    <b-card-text class="nft-price-value">{{
                      it.value.metadata.name
                    }}</b-card-text>
                  </div>
                </b-card-body>
              </router-link>
          ...
          <b-col v-if="items.length === 0 && loadingItems" class="is-flex">
            ...
            >
              ...
              <b-card-body class="is-flex justify-center">
                <div class="nft-head">
          ...
          <div class="no-item" v-if="items.length === 0 && !loadingItems">
            ...
            <p class="msg">NFTs will be shown here</p>
          </div>
          <!-- End skeleton loading -->
        </b-row>
      </div>
      <div
        v-if="loadingItems && 1 < page && limit * page < totalResults"
      ...
      <div>
        ...
        >
          <template>
            <div v-for="(trait, key) in Object.keys(stats)" :key="key">
              ...
              >
                <b-container>
                  <div
                    v-for="(it, k) in Object.keys(stats[trait])"
                    :key="k"
                    class="is-flex"
                    style="justify-content: space-between"
                  >
                    <div>
                      <b-form-group>
                        <b-form-checkbox
                          switch
                          v-model="filters[trait][it]"
                          v-on:change="
                            selects.push([trait, it, filters[trait][it]])
                          "
                          >{{ it }}</b-form-checkbox
                        >
                      </b-form-group>
          ...
          <template #modal-footer="{ ok, cancel }">
            ...
            <b-button class="btn-success" @click="ok()"> OK </b-button>
          </template>
        </b-modal>
      </div>
    </b-container>
    <b-modal
      id="import-token-alert"
      no-close-on-backdrop
      centered
      hide-footer
      title="Import tokens"
    >
      <ImportTokenAlert
        @import-success="$bvModal.hide('import-token-alert')"
        :tokens="{ ft: ft, nft: nft }"
      />
    </b-modal>
    <b-modal
      id="your-nft"
    ...
    >
      <YourNFT
        ...
        :ftSymbol="ftSymbol"
        :nftSymbol="nftSymbol"
        @nft-success="$bvModal.hide('your-fnt')"
      />
    </b-modal>
    <b-modal
      id="mint"
      ...
      title="Mint NFT"
    >
    <center><img :height="img?'200':''" :width="img?'200':''" :src="img" /></center>
    <b-form-group label="image file" label-for="file">
        
      <input type="file" accept="image/*" @change="onFileChange" />
      </b-form-group>
      <b-form-group label="title" label-for="price">
    ...
    >
      ...
      <BidMarket
        ...
        :nftAddress="nft"
        @bid-market-success="
          getMarketInfo().then(() => $bvModal.hide('liquidity'))
        "
        @bid-market-cancel="$bvModal.hide('liquidity')"
      />
      <Withdraw
        v-if="isWithdraw"
        :lpShare="lpShare"
        ...
        @withdraw-success="
          getMarketInfo().then(() => $bvModal.hide('liquidity'))
        "
        @withdraw-cancel="$bvModal.hide('liquidity')"
      />
    </b-modal>
    <b-toast
      id="tx-confirm"
    ...
    >
     ...
     <b-toast
      ...
      auto-hide-delay="8000"
    >
      Your transaction is successful. your token id <a :href="`/${tokenId}`">{{tokenId}}</a>
    </b-toast>
     <b-toast id="error-message" variant="danger" solid>
...
export default {
  ...
  data: () => ({
    ...
    currencyOptions: [{ value: "BAZA", text: "Currency: BAZA" }],
    nft: "",
    ft: "",
    ftLink: "",
    nftSymbol: "",
...
tokenId:""
  ...
  process.env.VUE_APP_CONTRACT,
  {
    ...
    viewMethods: ["getAllNFT","nft_total_supply","nft_metadata"], // view methods do not change state but usually return a value
    changeMethods: ["nft_mint"], // change methods modify state
    sender: this.wallet.account(), // account object to initialize and sign transactions.
  }
);
  const metadata=await this.contract.nft_metadata({});
  const total=await this.contract.nft_total_supply({});
  this.ftSymbol=metadata.symbol;
  this.ftName=metadata.name;
  this.next=total;
  ...
  methods: {
    async mintNow(){
      if(this.title.trim().length===0||this.desc.trim().length===0||this.img.trim().length===0){
      ...
      try{
this.isLoading=true;
           let jumlah=  await this.contract.nft_total_supply({});
      const imageName=`${uuidv4()}.${this.imgName.split(".")[this.imgName.split(".").length-1]}`;
       const joko=await fetch("/api/copin", {
               method: "POST",
               headers:{
                   "Content-Type": "application/json"
               },
               body:JSON.stringify({name:imageName,type:this.imgType})
    })
    const url=await joko.json();
    const response = await fetch(url.body, { headers:{'Content-Type': this.imgType},method: "PUT", body:this.file});
    if(response.ok){
      const res = await fetch("/api/head", { headers:{'Content-Type': "application/json"},method: "POST", body:JSON.stringify({key:imageName})});
      const data=await res.json();
      
      await this.contract.nft_mint({
         token_id: `${jumlah+1}`,
    metadata: {title:this.title,description:this.desc,media:`https://ipfs.filebase.io/ipfs/${data.Metadata.cid}`},
...
this.title="";
this.desc="";
jumlah=  await this.contract.nft_total_supply({});
const items=await this.contract.getAllNFT({prev:0,next:jumlah});
this.items=items;
     this.$bvModal.hide('mint')
  this.isLoading=false;
  this.$bvToast.show("berhasil");
  this.tokenId=jumlah;
      }catch(e){
        this.isLoading=false;
...
this.errorMessage=e.message;
    ...
    getData: async function () {
      try {
        let { data } = await axios.get(
          urljoin(
            config.apiBaseUrl,
            `/api/${this.chainId}/data/${this.nft}/all`
          )
        );
        this.stats = data.stats;
        this.filters = Object.keys(this.stats).reduce(
          (a, v) => ({ ...a, [v]: {} }),
          {}
        );
      ...
      try {
        let { data } = await axios.get(
          urljoin(
            config.apiBaseurl,
            `/api/${this.chainId}/markets/${this.marketAddress}`
          )
        );
    ...
    loadItems: async function () {
      let total =
        this.allData.length !== this.result.length
          ? this.result.length
          : this.totalSupply.toNumber();
      this.totalResults = total;
      let max = total > this.limit * this.page ? this.limit * this.page : total;
    ...
    async getPrices(it) {
      ...
      bestBid = bestBid
        ? ethers.BigNumber.from(bestBid)
        : (await this.market.getBestBid(it.tokenId))[1];
      bestBid = this.bestBidMarketPrice.gt(bestBid)
        ? this.bestBidMarketPrice
        : bestBid;
      let askPrice = this.askPrices ? this.askPrices[it.tokenId] : 0;
      askPrice = askPrice
        ? ethers.BigNumber.from(askPrice)
        : await this.market.getAskPrice(it.tokenId);
    ...
    async getMarketInfo() {
      let calls = await Multicall.setMulticall(
        this.ft,
        this.nft,
        this.provider
      );
      ...
      if (this.address) {
        let { price, share, isAuto, ROI } = await calls.getBidMarketInfo(
          this.address
        );
        this.lpPrice = price;
        this.lpShare = share;
